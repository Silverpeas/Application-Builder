...
public class TestXPath {
    ...
    public TestXPath() throws Exception {
		...
		xpathObj.setNodeAsStart();
		testACTION("@JNDIName");
		xpathObj.setValue("com.stratelia.silverpeas.backbone.soap.jms.QueueConnectionFactory");
		testACTION("@Targets");
		xpathObj.setValue("Serveur1");
	...
	private void testSyntax(String xpath) {
		...
		try {
			xpathObj.parse();
			System.out.println(" successfull (abs:"+xpathObj.isAbsolute()+";attrib:"+xpathObj.isAttribute()+")");
		}
	...
	private void testACTION(String xpath) {
		...
		try {
			...
			if (b!=null && b.booleanValue()) {
				System.out.println("EXISTS : "+xpathObj.getNode()+" "+xpathObj.getValue());
			} else {
